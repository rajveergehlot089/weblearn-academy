...
async function updateUser(userId, fields) {
  ...
  if (fields.role && ['customer', 'admin'].includes(fields.role)) {
    updates.push(`role = $${i++}`);
    params.push(fields.role);
  }
  if (fields.passwordHash) {
    updates.push(`"passwordHash" = $${i++}`);
...
async function setActiveCourse(userId, courseId) {
  ...
  await pool.query('UPDATE enrollments SET "activeCourse" = $1 WHERE "userId" = $2 AND "courseId" = $3', [
    courseId,
    userId,
    courseId,
  ]);
...
async function getTopicProgress(userId, courseId, topicId) {
  return one('SELECT * FROM topic_progress WHERE "userId" = $1 AND "courseId" = $2 AND "topicId" = $3', [
    userId,
    courseId,
    topicId,
  ]);
...
async function logDailyActivity(userId, date, minutes, topicId) {
  ...
  if (existing) {
    ...
    await run('INSERT INTO daily_log ("userId", date, "minutesSpent", "topicsVisited") VALUES ($1, $2, $3, $4)', [
      userId,
      date,
      minutes || 0,
...
async function saveTypingScore(userId, courseId, topicId, wpm, accuracy, timeLimit) {
  ...
  const existing = await one('SELECT * FROM typing_scores WHERE "userId" = $1 AND "courseId" = $2 AND "topicId" = $3', [
    userId,
    courseId,
    key,
  ]);
  ...
  return one('SELECT * FROM typing_scores WHERE "userId" = $1 AND "courseId" = $2 AND "topicId" = $3', [
    userId,
    courseId,
    key,
  ]);
...
async function getInterviewProgress(userId, courseId, topicId) {
  return all(
    'SELECT "questionIndex", correct FROM interview_progress WHERE "userId" = $1 AND "courseId" = $2 AND "topicId" = $3',
...
async function getExerciseProgress(userId, courseId, topicId) {
  return all(
    'SELECT "exerciseIndex", correct FROM exercise_progress WHERE "userId" = $1 AND "courseId" = $2 AND "topicId" = $3',
    [userId, courseId, topicId],
  );
}
...
async function saveAnalysis(userId, id, analysis) {
  await run('INSERT INTO analysis_history (id, "userId", analysis, "createdAt") VALUES ($1, $2, $3, $4)', [
    id,
    userId,
...
async function getAnalysisHistory(userId) {
  return all(
    'SELECT id, analysis, "createdAt" FROM analysis_history WHERE "userId" = $1 ORDER BY "createdAt" DESC LIMIT 20',
    [userId],
  );
}
...
async function getDailyActiveLast30Days() {
  ...
  const result = {};
  rows.forEach((r) => {
    result[r.date] = parseInt(r.count);
  });
  return result;
}
...
async function saveVerificationToken(token, userId, email, expiresAt) {
  await run('INSERT INTO verification_tokens (token, "userId", email, "expiresAt") VALUES ($1, $2, $3, $4)', [
    token,
    userId,
    email,
    expiresAt,
...
async function saveResetToken(token, userId, email, expiresAt) {
  await run('INSERT INTO reset_tokens (token, "userId", email, "expiresAt") VALUES ($1, $2, $3, $4)', [
    token,
    userId,
    email,
    expiresAt,
...
async function incrementFailedLogin(userId) {
  await run('UPDATE users SET "failedLoginAttempts" = "failedLoginAttempts" + 1 WHERE id = $1', [userId]);
}

async function resetFailedLogin(userId) {
  await run('UPDATE users SET "failedLoginAttempts" = 0, "lockedUntil" = NULL WHERE id = $1', [userId]);
}

async function lockAccount(userId, until) {
  await run('UPDATE users SET "lockedUntil" = $1 WHERE id = $2', [until, userId]);
}
...
async function incrementTokenVersion(userId) {
  await run('UPDATE users SET "tokenVersion" = "tokenVersion" + 1 WHERE id = $1', [userId]);
}
...
async function createCourse(data) {
  await run(
    ...
    [
      ...
      data.color || '#667eea',
      data.contentDir || data.id,
      data.hasTypingPractice ? 1 : 0,
      data.typingLayout || 'qwerty',
...
module.exports = {
  ...
  deleteAnalysis,
  getAdminStats,
  getDailyActiveLast30Days,
  getCourseTopicCompletions,
  // Security
